...
**Editor:** `micro` is the preferred terminal editor across all devices. `$EDITOR` is set to `micro` in `~/.zshrc`. When setting up a new environment, install with `brew install micro` and `export EDITOR="micro"`. Do not default to vim/vi in scripts or suggestions — use `micro` or respect `$EDITOR`.
...
| `/update` | Package + packageManager update workflow (single project) |
| `/update-all` | Update global CLI tools + all 7 projects sequentially |
