...
/**
 * Zendesk Query Testing Script - Comprehensive Test Suite
 *
 ...
 *
 * Usage:
 *   bun scripts/test-zendesk-queries.ts              # Run all tests
 *   bun scripts/test-zendesk-queries.ts --category=discrete  # Run only discrete tests
 *   bun scripts/test-zendesk-queries.ts --category=complex   # Run only complex tests
 *   bun scripts/test-zendesk-queries.ts --verbose    # Show full answers
 *   bun scripts/test-zendesk-queries.ts --json       # Output JSON results
 *
 * Test Categories:
...
import { handleSmartQuery } from "@/app/zendesk/lib/zendesk-smart-query-handler"
...
async function main() {
  ...
  if (!jsonOutput) {
    ...
    console.log("ZENDESK QUERY SYSTEM - COMPREHENSIVE TEST SUITE")
    console.log("=".repeat(70))
