...
const CACHE_FILE = "app/zendesk/cache/conversation-cache.json"
 ...
 */
export function loadConversationCache(): ConversationCache {
  try {
    ...
    console.error("[ConversationCache] Error loading cache:", error)
    return {
      entries: [],
      maxEntries: MAX_ENTRIES,
      lastUpdated: new Date().toISOString(),
 ...
 */
export function saveConversationCache(cache: ConversationCache): void {
  try {
    const fs = require("node:fs")
    const path = require("node:path")
    const cacheFilePath = path.join(process.cwd(), CACHE_FILE)
    ...
    console.error("[ConversationCache] Error saving cache:", error)
  }
}
...
export function clearConversationCache(): void {
  ...
  saveConversationCache(cache)
  console.log("[ConversationCache] Cache cleared")
}
