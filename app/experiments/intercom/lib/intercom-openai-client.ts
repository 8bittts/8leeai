/**
 * OpenAI client wrapper for interpreting complex Intercom queries
 * Uses the @ai-sdk/openai package and follows the pattern from /api/intercom/suggest-response
 */

import { openai } from "@ai-sdk/openai"
import { generateText } from "ai"
import type { QueryIntent } from "./intercom-types"
 ...
 */
const SYSTEM_PROMPT = `You are an expert at interpreting natural language queries for a Intercom support ticketing system.
...
export async function interpretComplexQuery(query: string): Promise<QueryInterpretationResult> {
  ...
  try {
    const { text } = await generateText({
      ...
      prompt: `Interpret this Intercom support query: "${query}"`,
