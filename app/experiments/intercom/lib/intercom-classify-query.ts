...
function tryBreakdownMatch(
  ...
  if (/\b(status|state)\b/i.test(q)) {
    const breakdown = Object.entries(cache.stats.byState)
      .map(([state, count]) => `**${state}**: ${count}`)
      .join(" | ")
    return {
      answer: `State breakdown: ${breakdown}`,
      confidence: 0.95,
      reasoning: "State distribution",
    }
  }
  ...
  if (/\b(tag|tags|ticket tag)\b/i.test(q)) {
    const breakdown = Object.entries(cache.stats.byTag)
      .map(([tag, count]) => `**${tag}**: ${count}`)
      .join(" | ")
    return {
      answer: `Tag breakdown: ${breakdown}`,
      confidence: 0.95,
      reasoning: "Tag distribution",
    }
  }
...
function tryTagMatch(
  ...
  if (tagMatch && /\b(tag(ged)?|tickets?|issues?)\b/i.test(q)) {
    // Use pre-calculated tag stats
    const count = cache.stats.byTag[tagMatch] || 0
    return {
...
function tryDiscreteMatch(
  ...
  // State query (specific) - Intercom uses "state" instead of "type"
  const stateMatch = DISCRETE_INDICATORS.ticketType.find((state) =>
    new RegExp(`\\b${state}\\b`, "i").test(q)
  )
  if (stateMatch && /\b(tickets?|issues?)\b/i.test(q)) {
    const count = cache.stats.byState[stateMatch] || 0
    return {
      answer: `Tickets in state **${stateMatch}**: ${count}`,
      confidence: 0.95,
      reasoning: `State filter: ${stateMatch}`,
    }
  }

  // Priority query (specific) - Intercom uses priority/noPriority
  const hasPriorityQuery = /\b(high\s*priority|priority|urgent)\b/i.test(q)
  const hasNoPriorityQuery = /\b(no\s*priority|normal|low)\b/i.test(q)
  if ((hasPriorityQuery || hasNoPriorityQuery) && /\b(tickets?|issues?)\b/i.test(q)) {
    const count = hasPriorityQuery
      ? cache.stats.byPriority.priority
      : cache.stats.byPriority.noPriority
    const label = hasPriorityQuery ? "priority" : "no priority"
    return {
      answer: `Tickets with **${label}**: ${count}`,
      confidence: 0.95,
      reasoning: `Priority filter: ${label}`,
    }
  }

  // Status query (specific) - Intercom uses "state" terminology
  ...
  if (statusMatch && /\b(tickets?|issues?)\b/i.test(q)) {
    const count = cache.stats.byState[statusMatch] || 0
    return {
      answer: `Tickets in state **${statusMatch}**: ${count}`,
      confidence: 0.95,
      reasoning: `State filter: ${statusMatch}`,
    }
  }
