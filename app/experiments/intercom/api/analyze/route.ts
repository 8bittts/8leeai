/**
 * Intercom Analysis Endpoint
 * POST /api/intercom/analyze
 *
...
function generateHelpText(): string {
  const help = `
INTERCOM SUPPORT AI - AVAILABLE COMMANDS
========================================
 ...
 */
async function getTicketStatusInfo(client: IntercomClient): Promise<string> {
  try {
    ...
    for (const [status, countValue] of Object.entries(stats)) {
      const count =
        typeof countValue === "number" ? countValue : Number.parseInt(String(countValue), 10) || 0
 ...
 */
async function getRecentActivity(client: IntercomClient): Promise<string> {
  try {
 ...
 */
async function analyzeProblemAreas(client: IntercomClient): Promise<string> {
  try {
 ...
 */
async function getRawData(client: IntercomClient): Promise<string> {
  try {
    const stats = await client.getTicketStats()
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.log(`[IntercomAnalyze] Intent: ${intent}, Query: "${query}"`)
    ...
    console.error("[IntercomAnalyze] Error:", error)
