/**
 * Comprehensive Intercom Intelligence Portal Test
...
/**
 * Execute a query against the Intercom API
 */
async function executeQuery(
  ...
  const response = await fetch(`${API_BASE}/intercom/api/query`, {
    method: "POST",
...
async function runTests(): Promise<void> {
  ...
  await test(
    ...
    "hello",
    (r) => r.answer.includes("Intercom Intelligence Assistant") && r.source === "cache"
  )
  ...
  if (testContext && testContext.length > 0) {
    ...
    await test(
      ...
      "build a reply for the first ticket and send it to intercom",
      (r) => {
        const hasReplyGenerated = r.answer.includes("Reply Generated") || r.answer.includes("reply")
        const hasTicketLink = r.answer.includes("intercom.com") || r.answer.includes("Direct Link")
        const hasCommentId = r.answer.includes("Comment ID") || r.answer.includes("comment")

        if (hasReplyGenerated) {
          console.log("   ✉️  Reply generated and posted to Intercom")
        }
        if (hasTicketLink) {
    ...
    await test(
      ...
      (r) =>
        ...
        r.answer.includes("intercom.com"),
      contextWithTickets
    )
