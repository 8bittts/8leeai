...
/**
 * Intercom Ticket Generator Script
 *
 * Creates realistic and varied support tickets directly in Intercom
 * with built-in rate limiting to respect API limits.
 *
 * Usage:
 *   bun scripts/generate-intercom-tickets.ts [options]
 *
 * Options:
 ...
 *
 * Examples:
 *   bun scripts/generate-intercom-tickets.ts                    # Generate 10 random tickets
 *   bun scripts/generate-intercom-tickets.ts --count 25         # Generate 25 tickets
 *   bun scripts/generate-intercom-tickets.ts --count 15 --delay 1000  # Slower generation
 */
...
/**
 * Create a ticket in Intercom via API
 */
async function createIntercomTicket(
  ticket: TicketData,
  index: number,
  options: ReturnType<typeof parseArgs>
): Promise<{ success: boolean; ticketId?: string; error?: string }> {
  const email = process.env["INTERCOM_EMAIL"]
  ...
  const url = `https://${subdomain}.intercom.com/api/v2/tickets.json`

  const payload: IntercomTicket = {
    ticket: {
...
async function main() {
  ...
  console.log("\n╔═══════════════════════════════════════════════════════════════════╗")
  console.log("║         INTERCOM TICKET GENERATOR - Create Realistic Tickets      ║")
  console.log("╚═══════════════════════════════════════════════════════════════════╝")
  ...
  console.log(`\n📬 Submitting tickets to Intercom (${options.delay}ms delay between requests)...`)
  for (let i = 0; i < tickets.length; i++) {
    const result = await createIntercomTicket(tickets[i], i, options)
    results.push(result)
  ...
  if (failed > 0) {
    ...
    console.log("   View them at: https://8lee.intercom.com/agent/dashboard")
    process.exit(0)
  }
}
