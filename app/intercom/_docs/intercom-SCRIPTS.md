...
This directory contains utility scripts for testing and managing the Intercom Intelligence Portal.
...
| `intercom-generate-tickets.ts` | Create realistic test tickets | `bun scripts/intercom-generate-tickets.ts [options]` |
| `intercom-full-workflow-test.ts` | Comprehensive integration test | `bun scripts/intercom-full-workflow-test.ts` |
| `intercom-api-test.ts` | API connectivity test | `bun scripts/intercom-api-test.ts` |
| `intercom-queries-test.ts` | Query classification test | `bun scripts/intercom-queries-test.ts` |
| `intercom-create-synthetic-tickets.ts` | Generate test tickets with metadata | `bun scripts/intercom-create-synthetic-tickets.ts [count]` |
| `intercom-add-ticket-metadata.ts` | Add metadata to existing tickets | `bun scripts/intercom-add-ticket-metadata.ts` |
| `intercom-generate-tickets-with-replies.ts` | Generate tickets with AI replies | `bun scripts/intercom-generate-tickets-with-replies.ts` |
...
### Purpose
Validates that both Intercom and OpenAI API credentials are correctly configured and authenticated.

### What It Does
- ✅ Checks all required environment variables are set
- ✅ Tests Intercom API authentication (makes actual API call)
...
✅ INTERCOM_API_TOKEN: xhUpLvStmz...OM7qQOGrKY
✅ INTERCOM_SUBDOMAIN: 8lee
✅ OPENAI_API_KEY: sk-proj-Dr...BiZbU-mwA

================================================
🧪 TESTING INTERCOM API
================================================

HTTP Status: 200

✅ INTERCOM API - AUTHENTICATED ✅
...
```bash
INTERCOM_EMAIL=your-admin-email@example.com
INTERCOM_API_TOKEN=your-api-token
INTERCOM_SUBDOMAIN=your-subdomain
OPENAI_API_KEY=sk-proj-your-key
```

### Troubleshooting

**Intercom returns 401 Unauthorized:**
- Verify email matches your Intercom admin account
- Check API token hasn't been revoked in Intercom admin panel
- Regenerate API token at: `https://your-subdomain.intercom.com/admin/apps-and-integrations/apis/intercom-api`
...
## 2. intercom-generate-tickets.ts

### Purpose
Generates realistic, varied support tickets directly in your Intercom account. Useful for testing analytics, UI rendering, and AI analysis features with actual data.

### What It Does
- 🎲 Generates realistic ticket data (names, emails, subjects, descriptions)
- 📤 Creates tickets directly in Intercom via API
...
```bash
bun scripts/intercom-generate-tickets.ts
```

**Create 100 tickets with slower rate limiting:**
```bash
bun scripts/intercom-generate-tickets.ts --count 100 --delay 1000
```

**Create 50 tickets with fixed priority:**
```bash
bun scripts/intercom-generate-tickets.ts --count 50 --priority high
```

**Create 25 tickets with all settings customized:**
```bash
bun scripts/intercom-generate-tickets.ts --count 25 --delay 750 --priority urgent --status open
```
...
```bash
bun scripts/intercom-generate-tickets.ts --count 100 --delay 500
# Takes ~50 seconds (100 tickets × 500ms delay)
```

**Quick load test with 50 high-priority urgent tickets:**
```bash
bun scripts/intercom-generate-tickets.ts --count 50 --priority high --delay 250
```

**Generate support tickets in pending status for workflow testing:**
```bash
bun scripts/intercom-generate-tickets.ts --count 30 --status pending
```
...
```
╔═══════════════════════════════════════════════════════════════════╗
║         INTERCOM TICKET GENERATOR - Create Realistic Tickets      ║
╚═══════════════════════════════════════════════════════════════════╝
...
📬 Submitting tickets to Intercom (500ms delay between requests)...
...
✅ All tickets created successfully!
   View them at: https://8lee.intercom.com/agent/dashboard
```
...
- **Rate limiting** (Intercom limits requests per minute)
...
bun scripts/intercom-generate-tickets.ts --count 50
```
...
- Check that Intercom account has API access enabled
...
- Intercom may cache results, refresh the dashboard
...
INTERCOM_EMAIL=your-admin-email@example.com
INTERCOM_API_TOKEN=your-intercom-api-token
INTERCOM_SUBDOMAIN=your-subdomain
...
**Intercom API Token:**
1. Go to: `https://your-subdomain.intercom.com/admin/apps-and-integrations/apis/intercom-api`
...
bun scripts/intercom-generate-tickets.ts --count 50 --delay 500

# 3. Start dev server
bun run dev

# 4. Access at http://localhost:1333/intercom
```

### Refresh Test Data
```bash
# Clear old tickets in Intercom manually, then:
bun scripts/intercom-generate-tickets.ts --count 100 --delay 500
```
...
bun scripts/intercom-generate-tickets.ts --count 200 --delay 250

# Then test:
# - http://localhost:1333/intercom
...
```bash
# All urgent tickets
bun scripts/intercom-generate-tickets.ts --count 30 --priority urgent
...
**test-credentials.sh:**
- Takes ~2-3 seconds
- Makes 2 HTTP requests (one to Intercom, one to OpenAI)
...
- Makes count HTTP requests to Intercom API
...
- This script doesn't call OpenAI (only the Intercom analysis endpoint does)
...
- Check Intercom dashboard after creation
...
- Assume instant appearance in Intercom dashboard (may cache)
...
- **intercom-generate-tickets.ts** - `/scripts/intercom-generate-tickets.ts` (400+ lines, TypeScript)
...
## 3. intercom-full-workflow-test.ts

### Purpose
Comprehensive integration test for the entire Intercom Intelligence Portal. Tests all query types, context awareness, two-way communication, and error handling.
...
**Or run directly:**
```bash
bun scripts/intercom-full-workflow-test.ts
```
...
- Validates reply posted to Intercom
...
```
╔═══════════════════════════════════════════════════════════════════╗
║   INTERCOM INTELLIGENCE PORTAL - COMPREHENSIVE TEST SUITE        ║
╚═══════════════════════════════════════════════════════════════════╝
...
🧪 Testing: Reply Request - First Ticket
   Query: "build a reply for the first ticket and send it to intercom"
   Source: ai
   Confidence: 95%
   Duration: 4532ms
   ✉️  Reply generated and posted to Intercom
...
**API Endpoints:**
- `/api/intercom/query` - All query types
- `/api/intercom/reply` - Reply generation and posting
...
- Run ticket generator first: `bun scripts/intercom-generate-tickets.ts --count 50`
- Refresh cache manually via UI: type "refresh"
- Check Intercom credentials are valid

**Reply tests fail:**
- Verify OpenAI API key is correct in `.env.local`
- Check Intercom API allows ticket updates
...
3. ✅ Verify Intercom account has API access enabled
...
7. ✅ Run comprehensive test: `bun run test:intercom`
